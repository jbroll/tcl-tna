...
foreach { type def } [array get ::tna::Types] {
    ...
    append ::tna::Typecase [subst -nocommands {
	 case TNA_$type: {
	    ...
	    if ( Tcl_Get${Type}FromObj(interp, Tcl_GetObjResult(interp), &value) != TCL_OK ) {
		return TCL_ERROR;
	    }
...
critcl::ccode [subst -nobackslashes -nocommands {
	...
	for ( x = s->ax[dims].star; x < ends; x += s->ax[dims].incr ) {
	    if ( Tcl_IsShared(X[dims]) ) {
		X[dims] = Tcl_NewLongObj(x);
	    } else {
	    ...
	    if ( !dims ) {
		if ( Tcl_EvalObjEx(interp, body, 0) != TCL_OK ) { return TCL_ERROR; }
...
oo::class create tna {
	...
	return "tna::get_$type[llength $dims] [tna::mkslice $data $dims [tna::indx $dims [join $indx ,]]] [join [lrange $axes 0 [llength $dims]-1]]"
    }
	...
	uplevel [list ::tna::sliceloop [set ::tna::TNA_$type]			\
			 [llength $dims]					\
			 [tna::mkslice $data $dims [tna::indx $dims $slice]]	\
			 $::tna::axes						\
			 "expr { $body }"]
    }
	...
	set items [red x $dims { set items [expr { $items*$x }] }]
 	...
 	return [my List-Helper [tna::mkslice $data $items [list [list 0 $items 1]] ] [lreverse $dims] 0]
    }
